...
import ApiClient
import GetAllMoviePopular
import GetAllMovieUpcoming
import PopularAdapter
import ResultAdapter
import ResultUpcoming
...
import com.example.challanges6.databinding.FragmentHomeScreenBinding
import com.rizqi.myapplication.fragments.HomeScreenFragmentDirections
    ...
    private fun showListMoviePopular(data: List<ResultAdapter>) {
        val adapter = PopularAdapter(object : PopularAdapter.OnClickListener {
            override fun onClickItem(data: ResultAdapter) {
                val id = data.id
                val toDetail =
                    HomeScreenFragmentDirections.actionHomeScreenFragmentToDetailFragment(id)
